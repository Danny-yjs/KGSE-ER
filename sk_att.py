...
class DSTANet(nn.Module):
    def __init__(self, num_class=11, num_point=70, num_frame=95, num_subset=3, dropout=0., config=None, num_person=1,
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 4, 3, 1, 2).contiguous().view(N * M, C, T, V)
        x = self.input_map(x)
